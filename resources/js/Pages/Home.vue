            ...
            <div class="d-flex justify-content-between align-items-center">
              <div>
                ...
                <small class="text-muted">{{
                  customerCreated !== null ? customerCreated : "Loading..."
                }}</small>
              </div>
            ...
            <div class="d-flex justify-content-between align-items-center">
              <div>
                ...
                <!-- <span class="text-success fw-semibold"
                                ...
                                > -->
                <small class="text-muted">{{
                  assetCreated !== null ? assetCreated : "Loading..."
                }}</small>
              </div>
              <div class="rounded-circle bg-danger p-3">
                <i
                  class="fas fa-shopping-basket text-white"
            ...
            <div class="d-flex justify-content-between align-items-center">
              <div>
                ...
                <!-- <span class="text-success fw-semibold"
                                ...
                                > -->
                <small class="text-muted">{{
                  licenseCreated !== null ? licenseCreated : "Loading..."
                }}</small>
              </div>
          ...
          <div class="card p-3 shadow-sm border-0 shadow-lg">
            <div class="d-flex justify-content-between align-items-center">
              <div>
                ...
                <h4 class="fw-bold">
                  {{ contractVendorCount !== null ? contractVendorCount : "" }}
                </h4>
                <!-- <span class="text-success fw-semibold"
                                    >+3%</span
                                > -->
                <small class="text-muted">{{
                  contractVendorCreated !== null ? contractVendorCreated : "Loading..."
                }}</small>
              </div>
          ...
          <table class="table align-middle">
            ...
            <tbody>
              <tr
                ...
                :key="index"
                :class="getRowClass(alert.duration)"
              >
                <td>
...
export default {
  ...
  data() {
    return {
      currentPage: 1,
      itemsPerPage: 3,
      customerCount: null, // Untuk menyimpan jumlah customer
      customerCreated: null,
      assetCount: null, // Untuk menyimpan jumlah asset
      assetCreated: null,
      licenseCount: null, // Untuk menyimpan jumlah license
      licenseCreated: null,
      contractVendorCount: null, // Untuk menyimpan jumlah contractVendor
      contractVendorCreated: null,
      alerts: [
        {
          category: "Asset",
    ...
    async fetch() {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/api/dashboard/fetch` // Menggunakan VUE_APP_ sebagai prefiks
        );

        if (response.data.success) {
          this.customerCount = response.data.render.customer_count;
          this.customerCreated = response.data.render.customer_created;
          this.assetCount = response.data.render.asset_count;
          this.assetCreated = response.data.render.asset_created;
          this.licenseCount = response.data.render.license_count;
          this.licenseCreated = response.data.render.license_created;
          this.contractVendorCount = response.data.render.contract_vendor_count;
          this.contractVendorCreated = response.data.render.contract_vendor_created;
        } else {
  ...
  mounted() {
    // Panggil fetchCustomer pertama kali
    this.fetch();

    // Jadwalkan panggilan fetchCustomer setiap 5 menit
    setInterval(() => {
      this.fetch();
