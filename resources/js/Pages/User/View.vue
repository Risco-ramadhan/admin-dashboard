        ...
        <div class="card my-5 shadow-lg rounded">
            <div class="container" v-if="user">
                ...
                <div class="row pt-3">
                    <div class="col-md-8">
                        <div class="row mb-3">
                            <label for="user_label" class="col-sm-4 col-form-label text-secondary">Username:</label>
                            <div class="col-sm-8">
                                <span id="user_label" class="form-control-plaintext fw-bold">{{ user.name }}</span>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label for="user_level" class="col-sm-4 col-form-label text-secondary">Level:</label>
                            <div class="col-sm-8">
                                <span id="user_level" class="form-control-plaintext fw-bold">{{ user.email }}</span>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label for="user_route" class="col-sm-4 col-form-label text-secondary">Role:</label>
                            <div class="col-sm-8">
                                <div v-if="roles && roles.length > 0">
                                    <span v-for="(role, index) in roles" :key="index" class="badge bg-primary me-1">
                                        {{ role }}
                                    </span>
                                </div>
                                <div v-else>
